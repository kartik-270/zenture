...
const getScoreInterpretation = (score: number) => {
  if (score >= 15) return { text: "Severe anxiety", color: "text-red-600" }
  if (score >= 10) return { text: "Moderate anxiety", color: "text-orange-600" }
  if (score >= 5) return { text: "Mild anxiety", color: "text-yellow-600" }
  return { text: "Minimal anxiety", color: "text-green-600" }
}
...
function GAD7Content() {
  const [answers, setAnswers] = useState<(number | null)[]>(new Array(gad7Questions.length).fill(null))
            ...
            {!showResults ? (
              <>
                ...
                <div className="mb-8">
                  <div className="flex justify-between text-sm text-muted-foreground mb-2">
                    <span>Question {step + 1} of {gad7Questions.length}</span>
                    <span>{Math.round(((step + 1) / gad7Questions.length) * 100)}%</span>
                  </div>
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-primary transition-all duration-300"
                      style={{ width: `${((step + 1) / gad7Questions.length) * 100}%` }}
                    />
                  </div>
                </div>
              ...
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm mb-8">
                  Score range: 0-21. This is a screening tool, not a diagnosis. 
